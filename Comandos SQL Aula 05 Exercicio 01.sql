...
select nm_funcionario, 
vl_salario_bruto, 
vl_salario_bruto * 1.05, 
vl_salario_bruto * 1.08 
from db_funcionario;
...
select * 
from db_pedido 
where EXTRACT(MONTH FROM dt_pedido) between 1 and 3 
and extract (year FROM dt_pedido) in (select max (extract (year from dt_pedido)) as Ano 
from db_pedido);
